...
// Pure derivation from raw tab fields → the context menu's panel
// target. Sync only: the menu handler runs inside the user-gesture
// stack (see permissions-model.md § gesture-stack), so it can't await
// the hostname search — the panel runs that itself.
