...
- [ ] Auto-sync toggle is **present** in the side-panel toolbar on
  ...
  button stays.
- [ ] Refresh button (D14): click → spinner runs, data re-fetches
  (bypasses the 24h cache), and a transient "Oppdatert" shows in the
  header. It reloads the *displayed* company only — it does NOT
  re-resolve the active tab or wipe the panel.
